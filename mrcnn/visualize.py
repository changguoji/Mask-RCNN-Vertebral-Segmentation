...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    save_path: (optional) The path to save figure.
    """
    # Number of instances
    N = boxes.shape[0]
    if not N:
    ...
    if fit_curve is not None:
        ax.add_line(lines.Line2D(fit_curve[1], fit_curve[0], linewidth=3, color='red'))
...
def display_instances_2(image, boxes0, boxes1, masks, class_ids, class_names,
                        scores=None, title="",
                        figsize=(16, 16), ax=None,
                        show_mask=True, show_bbox=True,
                        colors=None, captions=None,
    ...
    if not N:
        ...
        assert boxes0.shape[0] == masks.shape[-1] == class_ids.shape[0]
    ...
    masked_image0 = image.astype(np.uint32).copy()
    masked_image1 = image.astype(np.uint32).copy()
    for i in range(N):
        color = colors[i]

        # Bounding box
        if not np.any(boxes0[i]):
            # Skip this instance. Has no bbox. Likely lost in image cropping.
            continue
        ...
        mask = masks[:, :, i]
        if show_mask:
            masked_image0 = apply_mask(masked_image0, mask, color)
    ...
    ax[0].imshow(masked_image0.astype(np.uint8))
    ax[1].imshow(masked_image1.astype(np.uint8))
